...
void Image::SaveAsPPM(const std::string &filename) const
{
  ...
  constexpr auto kMultiplier = 256.f;
  for (int j = image_height - 1; j >= 0; --j) {
    for (int i = 0; i < image_width; ++i) {
      ...
      // Gamma correction + Clamping + Scaling to 0-255
      const auto ir = static_cast<int>(kMultiplier * Clamp(std::sqrt(color.x()), 0, 0.999f));
      const auto ig = static_cast<int>(kMultiplier * Clamp(std::sqrt(color.y()), 0, 0.999f));
      const auto ib = static_cast<int>(kMultiplier * Clamp(std::sqrt(color.z()), 0, 0.999f));
